...
if(isset($_POST['enviarLogin']) && $nombre != "" && $clave != ""){
	...
	if(compruebaUSU($conexion, $nombre, $clave)){
		...
		?><script> location.replace("filtradoPersonalizado.php"); </script><?php
	}else{
...
echo '<div><form action="index.php" method="POST">';
	echo '<input class="button" type="submit" id="inicio" name="inicio" value="Inicio"/>';
echo '</form>';

	echo '<div><form action="filtradoPersonalizado.php" method="POST">';
	echo '<br><input class="button" id="filtro_nombre" placeholder="Nombre" type="text" name="filtro_nombre" value=""/>';
	echo '<br><input class="button" id="filtro_apellido" placeholder="Apellidos" type="text" name="filtro_apellido" value=""/>';
	echo '<input class="button" type="submit" id="btn_filtro" name="enviaFiltro" value="Enviar"/>';
	echo '</form>';

	if(isset($_POST['enviaFiltro'])){


			function listaJugadores($conexion){
				$mconsulta = "SELECT * from jugadores WHERE nombre='".$_POST['filtro_nombre']."' AND apellidos='".$_POST['filtro_apellido']."'";
				...
				while($fila = mysqli_fetch_assoc($resultado)){
					...
					if (file_exists("jugadoresFiltroPersonalizado.json")) { unlink ("jugadoresFiltroPersonalizado.json"); }

					$mfile = fopen("jugadoresFiltroPersonalizado.json", "c+");
					fwrite($mfile, $mjson);
					fclose($mfile);
...
var url = "http://localhost/tenis/PEDROPABLO/Proyecto_Tenis/jugadoresFiltroPersonalizado.json";
