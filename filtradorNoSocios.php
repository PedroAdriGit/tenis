...
function listaJugadores($conexion){
	$mconsulta = "SELECT * from jugadores WHERE esSocio = 'no'";
	...
	/////// PRUEBAS //////
		$mjson = json_encode($array_general);
				...
				if (file_exists("jugadoresNoSocios.json")) { unlink ("jugadoresNoSocios.json"); }

		$mfile = fopen("jugadoresNoSocios.json", "c+");
		fwrite($mfile, $mjson);
		fclose($mfile);
